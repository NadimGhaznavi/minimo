<div align="center">
  ...
  your browser to the Git tag corresponding to your version. e.g. https://github.com/jekyll/minimo/blob/v2.5.0/README.md.<br/>
  Running <code>bundle show minimo</code> will provide you with the local path to your current theme version.</em></p>
  <img src="/readme_banner.svg"/>
  <p>It's Jekyll's default (and first) theme. It's what you get when you run <code>jekyll new</code>.</p>
  <p><a href="https://jekyll.github.io/minimo/">Theme preview</a></p>
...
Minimo has been scaffolded by the `jekyll new-theme` command and therefore has all the necessary files and directories to have a new Jekyll site up and running with zero-configuration.
...
From Minimo v3 onwards, the base layout is named **`base.html`** instead of `default.html` to avoid confusing new users into
...
From Minimo v2.2 onwards, the *home* layout will inject all content from your `index.md` / `index.html` **before** the **`Posts`** heading. This will allow you to include non-posts related content to be published on the landing page under a dedicated heading. *We recommended that you title this section with a Heading2 (`##`)*.

Usually the `site.title` itself would suffice as the implicit 'main-title' for a landing-page. But, if your landing-page would like a heading to be explicitly displayed, then simply define a `title` variable in the document's front matter and it will be rendered with an `<h1>` tag.

##### *Post Listing*

This section is optional from Minimo v2.2 onwards.<br/>
...
Refers to snippets of code within the `_includes` directory that can be inserted in multiple layouts (and another include-file as well) within the same theme-gem.
  ...
  - `social.html` &mdash; Renders social-media icons based on the `minimo:social_links` data in the config file.
...
Refers to `.scss` files within the `_sass` directory that define the theme's styles.
  ...
  - `minimo/skins/classic.scss` &mdash; The "classic" skin of the theme. *Used by default.*
  - `minimo/initialize.scss` &mdash; A component that defines the theme's *skin-agnostic* variable defaults and sass partials.
    It imports the following components (in the following order):
    - `minimo/custom-variables.scss` &mdash; A hook that allows overriding variable defaults and mixins. (*Note: Cannot override styles*)
    - `minimo/_base.scss` &mdash; Sass partial for resets and defines base styles for various HTML elements.
    - `minimo/_layout.scss` &mdash; Sass partial that defines the visual style for various layouts.
    - `minimo/custom-styles.scss` &mdash; A hook that allows overriding styles defined above. (*Note: Cannot override variables*)
...
Refers to various asset files within the `assets` directory.
  ...
  - `assets/minimo-social-icons.html` &mdash; Imports enabled social-media icon graphic and gets processed into a composite SVG file.
    Refer [section on social networks](#social-networks) for its usage.


### Plugins

Minimo comes with [`jekyll-seo-tag`](https://github.com/jekyll/jekyll-seo-tag) plugin preinstalled to make sure your website gets the most useful meta tags. See [usage](https://github.com/jekyll/jekyll-seo-tag#usage) to know how to set it up.
...
```yaml
remote_theme: NadimGhaznavi/minimo
```


### Customizing templates

To override the default structure and style of minimo, simply create the concerned directory at the root of your site, copy the file you wish to customize to that directory, and then edit the file.
e.g., to override the [`_includes/head.html `](_includes/head.html) file to specify a custom style path, create an `_includes` directory, copy `_includes/head.html` from minimo gem folder to `<yoursite>/_includes` and start editing that file.

The site's default CSS has now moved to a new place within the gem itself, [`assets/css/style.scss`](assets/css/style.scss).

In Minimo 3.0, if you only need to customize the colors of the theme, refer to the subsequent section on skins. To have your
*CSS overrides* in sync with upstream changes released in future versions, you can collect all your overrides for the Sass
variables and mixins inside a sass file placed at `_sass/minimo/custom-variables.scss` and all other overrides inside a sass file
placed at path `_sass/minimo/custom-styles.scss`.
...
@import
  "minimo/skins/{{ site.minimo.skin | default: 'classic' }}",
  "minimo/initialize";
```

#### Skins

Minimo 3.0 supports defining and switching between multiple color-palettes (or *skins*).
...
A skin is a Sass file placed in the directory `_sass/minimo/skins` and it defines the variable defaults related to the "color"
...
The default color palette for Minimo is defined within `_sass/minimo/skins/classic.scss`. To switch to another available skin,
simply declare it in the site's config file. For example, to activate `_sass/minimo/skins/dark.scss` as the skin, the setting
would be:
...
You can change the default date format by specifying `site.minimo.date_format`
in `_config.yml`.
...
From `Minimo-3.0` onwards, `site.author` is expected to be a mapping of attributes instead of a simple scalar value:
...
From `Minimo-3.0` onwards, the social media data is sourced from config key `minimo.social_links`. It is a list of key-value pairs, each entry
...
as `platform`. The `platform` key corresponds to the SVG id of the sprite in the composite file at URL `/assets/minimo-social-icons.svg`.
...
entry under key `minimo.social_links`.
...
Bug reports and pull requests are welcome on GitHub at https://github.com/jekyll/minimo. This project is intended to be a safe, welcoming space for collaboration, and contributors are expected to adhere to the [Contributor Covenant](http://contributor-covenant.org) code of conduct.
